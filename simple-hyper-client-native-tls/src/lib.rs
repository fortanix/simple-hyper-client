...
impl HttpsConnector {
    ...
    async fn connect(
        ...
        if is_https {
            let tls = tls
                .connect(&host, http.into_tcp_stream())
                .await
...
mod tests {
    ...
    fn get_tls_connector() -> TlsConnector {
        let pem = include_bytes!("../../test-ca/ca.cert");
    ...
    fn get_tls_acceptor() -> TlsAcceptor {
        let pem = include_bytes!("../../test-ca/end.fullchain");

        let key = include_bytes!("../../test-ca/end.key");

        let identity = Identity::from_pkcs8(pem, key).unwrap();
